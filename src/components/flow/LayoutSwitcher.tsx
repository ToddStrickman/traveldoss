...
export function LayoutSwitcher({
  ...
  return (
    <>
      {/* Gradient shell: drifting champagne-gold sand grains riding the
          perimeter. The gradient lives on a padded wrapper so the ring reads
          as a 1px border without a border-image. */}
      <span className="td-sand-shell relative inline-flex rounded-full p-px">
        <SandBorder />
        <button
          type="button"
          ...
          aria-expanded={open}
          className="tap relative inline-flex min-h-11 items-center gap-2.5 rounded-full bg-paper/85 px-4 text-ink/70 backdrop-blur-sm transition-colors duration-300 hover:bg-seal/5 hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-seal/60 motion-reduce:transition-none"
        >
          ...
          <span aria-hidden className="h-3 w-px bg-seal/30" />
          <span className="text-[10px] font-medium uppercase tracking-[0.2em] text-seal">
            {active.label}
          </span>
          <ChevronDown aria-hidden className="h-3.5 w-3.5 text-seal/70" />
